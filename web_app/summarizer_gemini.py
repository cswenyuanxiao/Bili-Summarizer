...
def summarize_content(file_path: Path, media_type: str, progress_callback=None, focus: str = "default", uploaded_file=None, custom_prompt: Optional[str] = None, output_language: str = "zh", enable_cot: bool = False) -> str:
    ...
    if custom_prompt:
        ...
        prompt_text = (
            ...
            "要求：\n"
            "1. 如果视频有视觉画面，请结合画面信息提供更丰富的描述。\n"
            "2. 摘要需要清晰、结构化且全面，避免流水账。\n"
            "3. 对关键概念、专有名词、方法或论点必须展开解释，补充背景、原因、步骤或证据。\n"
            "4. 保持客观，不添加原视频之外的臆测。\n"
            "5. 【重要】必须在总结的末尾提供一个思维导图，请使用标准 Markdown 无序列表，不要使用 Mermaid 或编号列表。\n"
            "6. 思维导图请以【思维导图】为标题，后续只输出无序列表，不要额外解释。\n"
            "7. 禁止使用客套开场（如“好的/当然/下面是/这是对视频的总结”）。\n"
            ...
            "- 核心定义：它是什么？\n"
            "- 深度细节：视频提供了哪些证据/数据/背景/步骤？\n"
            "- 重要性：为什么关键？导致了什么结果？\n\n"
            "# 💡 亮点与金句 (Highlights & Quotes)\n"
            "列出发人深省的观点、转折点或数据支撑（如有）。\n\n"
            "# 📝 总结与启示 (Conclusion & Takeaways)\n"
            "给出基于视频内容的结论、建议或启示（必须有内容，禁止空段）。\n\n"
            "11. 无序列表示例：\n"
            "- 核心主题\n"
            "  - 分支1\n"
            "    - 子点1\n"
            "    - 子点2\n"
            "  - 分支2\n"
            f"12. **重要**：请用 {target_language} 输出以下所有内容（包括总结、思维导图节点文本）。\n"
            "13. 【思维导图】必须使用 Markdown 无序列表（每行以 \"- \" 开头，子级用两个空格缩进）。禁止输出纯文本缩进列表。\n"
            "14. **数据可视化**（可选）：如果视频包含统计数据、对比或趋势，请在思维导图之后输出图表：\n"
            "```json\n"
            '{"charts": [{"type": "bar", "title": "标题", "data": {"labels": ["A"], "values": [10]}}]}\n'
            "```\n"
            "type可选: bar/pie/line。\n"
            "15. **关键词**（必填）：在图表之后输出关键词 JSON（词云用），格式如下：\n"
            "```json\n"
            '{"keywords": [{"text": "AI", "value": 10}, {"text": "Python", "value": 8}]}\n'
            "```\n"
            "value 为 1-10 权重。\n"
            "16. **内容深度要求**：对于中长视频或信息密度高的内容，正文必须足够详尽，不要过度概括。请保留重要的论据、案例、数据和逻辑推导过程。"
        )
