...
| 🧠 **思维导图** | 自动生成 Markdown 列表的知识结构图（Markmap 渲染），可导出 SVG/PNG |
...
| 功能 | 描述 | 状态 |
|------|------|------|
| 🌐 **多语言输出** | 支持中/英/日/韩/西/法 6 种语言的智能总结 | ✅ 已上线 |
| 🧠 **思维链展示** | 实时查看 AI 分析过程（内容识别→结构分析→总结策略） | ✅ 已上线 |
| 📊 **智能图表** | 自动识别视频数据并生成可视化图表 | ✅ 已上线 |
| 📚 **知识库 RAG** | 上传 PDF/文档，AI 结合外部知识进行总结 | 📅 规划中 |
...
### v2.2 🚧 开发中
- [x] **移动端 PWA** - 支持安装到桌面/手机，离线访问 (Beta)
- [/] **浏览器插件** - Chrome Extension MVP (快捷跳转分析)
...
- **文档权威入口**：`docs/AI_CONTEXT.md`
- **文档索引**：`docs/README.md`
- **变更同步规则**：任何 API/配置/部署变更必须同步对应文档与 `docs/CHANGELOG.md`
